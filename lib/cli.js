...
function correctOptions(options)
{
	...
	if(options.command === "analyze" && options.endpoint === "analysis")
	{
		if(options.params.parameters) options.params.parameters = JSON.parse(options.params.parameters);
		if(options.params.start) options.params.start = parseInt(options.params.start);
		if(options.params.end) options.params.end = parseInt(options.params.end);
	}
}
