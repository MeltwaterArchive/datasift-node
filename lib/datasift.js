...
// 
var DataSift = function (username, apikey, url_base, use_https, strict, version, in_url_base) {
	if (typeof use_https === "undefined" || use_https === null) { 
		use_https = true; 
	}
	...
	this.apikey = apikey;
	this.version = version || 1.2;
	this.url_base = (use_https) ? "https": "http";
	this.url_base = this.url_base + "://" + (url_base || "api.datasift.com") + "/v" + this.version;
	this.in_url_base = (use_https) ? "https": "http";
	this.in_url_base = this.in_url_base + "://" + (in_url_base || "in.datasift.com");
	this.definition = require('./version/' + this.version + '.json');
	this.strict = !strict ? false : true;
	this._setupRoutes();
...
DataSift.prototype._dispatch = function () {
	...
	// parse the URL for special params
	var uri = route.uri.replace(/\{\{(.*?)\}\}/g, function (x) {
		var replaced = '';
		if (x) {
			x = x.replace('{{', '').replace('}}', '');
			// look in the params object
			route.params.forEach(function (param) {
				if (param.name === x) {
					replaced = params[param.name];
				}
			});
		}
		return replaced;
	...
	this._request(route.method.toUpperCase(), this.url_base + uri, params, null, callback);

};

DataSift.prototype._request = function (method, uri, params, data, callback) {
...
DataSift.prototype.ingest = function(sourceId, data, callback) {

	if (!sourceId || !data) {
		throw 'sourceId & data arguments are required';
	}

	this._request('POST', this.in_url_base + "/" + sourceId, null, data, callback);
